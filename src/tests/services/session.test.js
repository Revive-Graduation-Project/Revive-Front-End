...
describe("Session Logic & Security Tests", () => {
  ...
  // TEST 2: Login preserves server-provided expiry
  test("login preserves server-provided expiry timestamp", async () => {
    const fakeUser = { id: 1, email: "test@test.com" };
    const fakeToken = "secret_access_token";
    const fakeExpiry = Date.now() + 20000;
    ...
    expect(useAuthStore.getState().expiresAt).toBe(fakeExpiry);
    console.log("✅ Login preserves server-provided expiry");
  });

  // TEST 3: restoreSession Success
  ...
  // TEST 5: restoreSession Skipped if not needed
