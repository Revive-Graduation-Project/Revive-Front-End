...
const RegularFoodCard = ({ meal }) => {
  ...
  return (
    <>
      <div
        id={`dish-card-${meal.id}`}
        className="group relative bg-white rounded-xl sm:rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 w-full flex flex-col overflow-hidden border border-gray-100"
      >
        ...
        <div className="relative flex justify-center pt-5 sm:pt-7 md:pt-8 pb-2 sm:pb-3 px-3 sm:px-4">
          ...
          {/* Circular food image — clickable to inspect full dish details */}
          <div
            onClick={() => setShowDetails(true)}
            title="Click to view full dish details"
            className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-full overflow-hidden border-2 sm:border-4 border-orange-50 shadow-md group-hover:scale-105 transition-transform duration-300 shrink-0 cursor-pointer relative"
          >
            <img
              src={imageUrl}
              alt={name}
        ...
        <div className="px-2 sm:px-3 md:px-4 pb-3 sm:pb-4 flex flex-col flex-1 gap-1 sm:gap-1.5">
          ...
          <h3
            onClick={() => setShowDetails(true)}
            title="Click to view full dish details"
            className="text-xs sm:text-sm md:text-base lg:text-lg font-bold text-gray-900 text-center line-clamp-1 cursor-pointer hover:text-orange transition-colors"
          >
            {name}
          </h3>
          ...
          <p
            onClick={() => setShowDetails(true)}
            className="text-[10px] sm:text-xs text-gray-400 text-left line-clamp-2 leading-snug min-h-8 sm:min-h-10 cursor-pointer"
          >
            {description}
          </p>
          <button
            type="button"
            onClick={() => setShowDetails(true)}
            className="text-[10px] sm:text-xs text-orange hover:text-orange-600 underline font-medium text-left cursor-pointer -mt-0.5"
          >
          ...
          <div
            onClick={() => setShowDetails(true)}
            className="grid grid-cols-2 gap-x-2 sm:gap-x-4 gap-y-0.5 text-[10px] sm:text-xs mt-0.5 cursor-pointer"
          >
