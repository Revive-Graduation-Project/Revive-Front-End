...
function OrdersView() {
  ...
  if (hasError) {
    ...
    return (
      <div>
        <DashboardHeader title="Orders" />
        <ErrorState message="Failed to load orders data." onRetry={handleRetry} />
      </div>
    );
  }
        ...
        <div className="flex-1 flex flex-col gap-6 w-full min-w-0">
          ...
          <div className="bg-white rounded-3xl shadow-sm py-4 px-2">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between px-4 pb-4 border-b border-gray-100 gap-4 sm:gap-0">
              ...
              <SortMenu columns={ORDER_SORT_COLS} sortKey={sortKey} sortDir={sortDir} onChange={(k, d) => { setSortKey(k); setSortDir(d); }} />
            </div>
              ...
              <table className="w-full border-collapse min-w-[700px]">
                ...
                <tbody>
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={7}>
                        <EmptyState title="No orders found" description={`No orders match "${activeTab}".`} />
                      </td>
                    </tr>
                  )}
                  {filtered.map((order) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        ...
                        <span className="block text-[10px] font-semibold text-orange-400 mt-1 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-0.5 group-hover:translate-y-0">
